...
const recentPosts = [
  { id: 1, name: 'Serial Communication with python', href: '#' },
  { id: 2, name: 'Infinite Runners in Godot', href: '#' },
  { id: 3, name: 'tailwind components repo update', href: '#' },
]
          ...
          <div className="hidden items-center justify-end md:flex md:flex-1 lg:w-0">
            {my_socials.map((item) => (
                <a
                    key={item.name}
                    href={item.href}
                    className="-m-3 flex items-center rounded-md mx-10 hover:bg-gray-50"
                >
                    <item.icon className="h-6 w-6 flex-shrink-0 text-green-600" aria-hidden="true" />
                </a>
                ))}
